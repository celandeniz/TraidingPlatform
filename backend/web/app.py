...
async def backtest(body: BacktestBody) -> dict:
    ...
    return {
        ...
        "profit_factor": float(res.profit_factor), "max_drawdown_pct": round(res.max_drawdown_pct, 1),
        "sharpe": float(res.sharpe), "buy_hold_pct": round(res.buy_hold_pct, 2),
        "excess_vs_buy_hold": round(res.excess_vs_buy_hold, 2),
        "exposure_pct": round(res.exposure_pct, 1), "error": res.error,
        # cast numpy scalars -> native types: pydantic v2 cannot serialize numpy.bool_
        "p_value": float(res.p_value), "significant": bool(res.significant),
        "ci_low_pct": float(res.ci_low_pct), "ci_high_pct": float(res.ci_high_pct),
